...
def get_report_data(prefix = "reports"):
...
def get_report_table_from_db():
	...
	df_db = pd.read_sql("SELECT * FROM report;", conn = engine)
	
	return(df_db)


def put_report_data(df_field, df_db):
	...
	try: new_df_field.to_sql('report', if_exists = 'append', conn = engine, chunksize = 100)
